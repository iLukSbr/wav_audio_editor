...
void fade (double* dados, int n_amostras, int inicio, int fim, int in_out)
{
    ...
    // fade out, de inicio ate fim
    if(in_out < 0)
        for(i = inicio, j = tam; i <= fim && i < n_amostras; i++, j--)// ate fim ou ate acabar dados
            dados[i] *= (double) j/tam;// reduz dados proporcional a posicao entre fim e inicio, de 100% a 0

    // fade in, de inicio ate fim
    else if(in_out > 0)
        for(i = inicio, j = 0; i <= fim && i < n_amostras; i++, j++)// ate fim ou ate acabar dados
            dados[i] *= (double) j/tam;// reduz dados proporcional a posicao entre fim e inicio, de 0 a 100%
}
...
int buscaPadrao (double* dados, int n_amostras, double* padrao, int n_amostras_padrao, double* score)
{
    ...
    for(i = 0; i < n_amostras; i++)// do inicio ao fim do dados
    {
        ...
        // diferenca entre numerador e denominador, quanto mais proximo de 0, melhor o score, que aproxima-se de 1
        dif = score_buffer - deno;
        if(dif < 0)
            dif *= -1;

        // se menor diferença entre numerador e denominador ou primeira análise
